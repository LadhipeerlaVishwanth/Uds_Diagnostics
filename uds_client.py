...
os.makedirs(
    os.path.join(BASE_DIR, "output", "can_logs"),
    exist_ok=True
)

os.makedirs(
    os.path.join(BASE_DIR, "output", "html_reports"),
    exist_ok=True
)
...
os.makedirs(
    os.path.join(BASE_DIR, "supportfiles"),
   exist_ok=True
)

#os.makedirs(
#    os.path.join(BASE_DIR, "input", "application"),
#    exist_ok=True
#)
...
class UDSClient:
    def __init__(self, config):
        ...
        log_dir = os.path.join(BASE_DIR,'output','can_logs')
    ...
    def run_testcase(self, oled):
        ...
        for tc_id, steps in grouped_cases.items():
            ...
            for step in steps:
                tc_id, step_desc, service, subfunc, expected, write_data, addressing, delay = step
                ...
                try:			 
                    ...
                    if not immediate_mode:
                         ...
                         service_delay = float(self.step_delays.get(delay_key,self.default_delay))
                         oled.display_centered_text(f"{tc_id}\n{step_desc[:20]}\n{status}")
                         time.sleep(service_delay)
                    if delay and str(delay).strip():
                         delay = str(delay).lower().strip()
                         if delay.endswith("ms"):
                              total_wait = float(delay[:-2]) / 1000
                         elif delay.endswith("s"):
                              total_wait = float(delay[:-1])
                         else:
                              total_wait = 0
                         start = time.time()
                         logging.info(f"{tc_id} VT Delay Started ({delay})")
                         while (time.time() - start) < total_wait:
                              try:# keep session alive
                                   with Client(self.active_conn["conn"], request_timeout=1, config = self.active_conn["client_config"]) as keep_alive:
                                        keep_alive.tester_present() 
                              except Exception:
                                   pass
                              if oled:
                                   remain = max(0,total_wait - (time.time() - start))
                                   oled.display_centered_text(f"{tc_id}\nDelay\n{remain:.1f}s")
                              time.sleep(0.5)
                except Exception as e:
                     if 'failure_reason' in locals():
                          ...
                          logging.error(f"----------------------------------{tc_id} {step_desc} -> Delay Error - {str(e)}-----------------------------------")
