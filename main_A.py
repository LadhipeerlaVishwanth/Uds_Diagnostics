...
from drivers.installer import Installer
...
class UDSApp:
    def __init__(self):
        ...
        self.installer = Installer(BASE_DIR)
        self.initialize_dependencies()

        
    
    def git_setup(self):
        self.oled.loading_dots("Checking\nRepository", 2)
        try:
    ...
    def initialize_dependencies(self):
        ...
        if self.installer.is_first_run():
            self.oled.display_centered_text("First Time\nSetup")
            time.sleep(2)
            success = self.installer.install(self.oled)
            if not success:
                self.oled.display_centered_text("Restart\nRequired")
                time.sleep(3)
                os._exit(1)
        self.btn_map = {
            "first": 12,
        ...
        max_retries = 3
        udp_connected = False
        self.oled.set_udp_status("U~")
        for retry in range(1, max_retries + 1):
            try:
                self.oled.set_udp_status("U~")
                self.oled.spinner(f"Checking\nUDP Server\n\n{retry}/{max_retries}",1)
        ...
        if not self.initialized_once:
            ...
            self.oled.loading_dots("Welcome\nDiagnostics",3)
        ...
        if can_config.get("bringup_on_startup", False):
            while time.time() - start_time < timeout_seconds:
                ...
                try:
                    self.oled.spinner("Initializing\nCAN",2)
                    ...
                    if success:
                        ...
                        self.oled.loading_dots("Waiting\nfor ECU", 2)
                except Exception as e:
    ...
    def wifi_setup(self):
        ...
        self.oled.loading_dots("Scanning\nWiFi", 2)
    ...
    def main_menu(self):
        while True:
            ...
            if selected_option == "ECU Information":
                ...
                self.oled.loading_dots("Uploading\nReports",2)
                try:
