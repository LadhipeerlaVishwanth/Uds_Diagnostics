...
class UDSClient:
    def __init__(self, config):
        ...
        self.physical_conn = self._create_connection(
            addr_modes_cfg.get("physical"), can_cfg, isotp_cfg, "physical"
        )
        self.functional_conn = self._create_connection(
            addr_modes_cfg.get("functional"), can_cfg, isotp_cfg, "functional"
        )
    ...
    def get_testcase_file_path(self):
        ...
        if not txt_files:
            raise FileNotFoundError(
                f"No .txt testcase file found in {support_dir}"
            )
    ...
    def run_testcase(
        self, oled
    ):  # sourcery skip: invert-any-all, use-fstring-for-concatenation
        def wait_for_final_response(client, tc_id, step_desc, timeout_total=3.0):
            response = client.conn.wait_frame(timeout=2)
            time.sleep(0.05)
            start_time = time.time()
            while (
                response and response[0] == 0x7F and response[2] == 0x78
            ):  # time.sleep(1)
                logging.info(
                ...
                if time.time() - start_time > max_wait:
                    logging.warning(
                        f"{tc_id} {step_desc} -> Timed out waiting after 0x78"
                    )
                    break
            return response
        ...
        for tc_id, steps in grouped_cases.items():
            ...
            for step in steps:
                if step[0] == "WAIT":
                    ...
                    oled.display_centered_text(f"WAIT\n{wait_ms/1000:.1f}s")
                    time.sleep(wait_ms / 1000)
                    continue
                if step[0] == "AWAIT_VALUE_MATCH":
                    expected_value = int(step[1], 16)
                    timeout_ms = step[2]
                    start_time = time.time()
                    matched = False
                    logging.info(f"AWAIT_VALUE_MATCH expected={hex(expected_value)} timeout={timeout_ms}")
                    oled.display_centered_text(f"AWAIT\n{hex(expected_value)}")
                    with Client(
                        ...
                        while (time.time() - start_time) * 1000 < timeout_ms:
                            try:
                                if self.last_request:
                                    self.active_conn["conn"].send(self.last_request)
                                    response = wait_for_final_response(
                                        client,
                                        tc_id,
                                        "AWAIT_VALUE_MATCH"
                                    )
                                    ...
                                    if response:
                                        actual_value = response[-1]
                                        logging.info(
                                            f"Received value {hex(actual_value)}"
                                        )
                                        if actual_value == expected_value:
                                            matched = True
                                            logging.info(
                                                f"Value matched {hex(actual_value)}"
                                            )
                                            oled.display_centered_text(
                                                f"MATCHED\n{hex(actual_value)}"
                                            )
                                ...
                                logging.error(
                                    f"AWAIT polling error: {e}"
                                )
                            time.sleep(0.2)
                    if not matched:
                        status = "Fail"
                        failure_reason = f"Timeout waiting for {hex(expected_value)}"
                        logging.error(
                            f"Timeout waiting for {hex(expected_value)}"
                        )
                        oled.display_centered_text(
                            f"Timeout\n{hex(expected_value)}"
                        )
                    ...
                    with Client(
                        ...
                        if service_int == 0x10:
                            ...
                            if subfunc != "":
                                ...
                                raw_request = bytes(
                                    [0x19, subfunc_int, status_mask_int]
                                )
                                logging.info(
                                    f"{tc_id} - {step_desc}: Sending {raw_request.hex().upper()}"
                                )
                        ...
                        # ClearDiagnosticInformation
                            ...
                            try:
                                ...
                                logging.error(
                                    f"{tc_id} - Invalid hex in subfunction: {ve}"
                                )
                            except Exception as e:
                                logging.error(
                                    f"{tc_id} - Exception: {type(e).__name__} - {str(e)}"
                                        ...
                                        for attempt in range(1, max_retries + 1):
                                            try:
                                                ...
                                                if len(key) != expected_key_length:
                                                    logging.info(f"Invalid key length ")
                                                self.context[
                                                    f"key_{subfunc_int + 1}"
                                                ] = key
                                                logging.info(
                                                    f"Received Key (for subfunc {hex(subfunc_int + 1)}): {key}"
                                                )
                                                break
                                                ...
                                                if attempt < max_retries:
                                                    ...
                                                    raise Exception(
                                                        f"Timeout after {max_retries} retries waiting for key from PC"
                                                    )
                                            except Exception as e:
                                                logging.exception(
                            ...
                            if subfunc != "":
                                try:
                                    ...
                                    if communication_type:
                                        try:
                                            communication_type_int = int(
                                                communication_type, 16
                                            )
                                        except ValueError:
                                            logging.warning(
                                                f"{tc_id} - Invalid communication_type '{communication_type}', defaulting to control_type"
                                            )
                                            communication_type_int = control_type
                                    else:
                                        communication_type_int = (
                                            control_type  # Fallback
                                        )

                                    raw_request = bytes(
                                        [0x28, control_type, communication_type_int]
                                    )
                                    logging.info(
                                        f"{tc_id} - {step_desc}: Sending raw request {raw_request.hex().upper()}"
                                    )
                                    ...
                                    logging.error(
                                        f"{tc_id} - Invalid subfunction format: '{subfunc}'"
                                    )
                                except Exception as e:
                                    logging.error(
