            ...
            for root, dirs, files in os.walk(usb_mount_path):
                for file in files:
                    if file.lower().endswith(".txt"):
                        testcase_src = os.path.join(root, file)
                        testcase_dst = os.path.join(BASE_DIR, "supportfiles", file)
                        ...
                        self.show_progress_animation(f"Copying {file}")
                        is_testcase_copied = True
                    elif file.lower().endswith(".json") and file != "git_credentials.json":
                        config_src = os.path.join(root, file)
                        config_dst = os.path.join(BASE_DIR, file)
