                                    ...
                                    try:
                                        for attempt in range(1, max_retries + 1):
                                            try:
                                                ...
                                                logging.info(f"Destination IP : {udp_ip}")
                                                logging.info(f"Destination Port : {udp_port}")
                                                logging.info(f"Seed : {seed.hex()}")
                                                sock.sendto(
                                                    seed.hex().encode(),
                                                    (udp_ip, udp_port),
                                                )
                                                logging.info("Seed sent successfully. Waiting for key...")
                                                key, _ = sock.recvfrom(1024)
                                                logging.info(f"Received {len(key)} bytes")
                                                logging.info(f"Raw key : {key.hex()}")
                                                key = key.strip()
                                                if not key:
                                                ...
                                                logging.info(f"Received Key (for subfunc {hex(subfunc_int + 1)}): {key.hex()}")
                                                self.udp_server_available = True
                                                break
